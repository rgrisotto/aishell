...
- **4.2.0 still publishes the bridging assets `aishell`, `aishell.bat` and `aishell.sha256`**: the 4.1.0 entry below says 4.2.0 would stop publishing them, but this release was cut for the Copilot Harness and the update-policy work and left the legacy gate in the build script on. So a v4.0.0 install can `aishell upgrade` straight to 4.2.0, and 4.3.0 is the release that drops the three assets. `docs/adr/0007-binary-distribution-by-appending-an-uberjar-to-babashka.md` carries the amendment
